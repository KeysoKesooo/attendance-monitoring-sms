...
class  Media {
  ...
  public $studentPath = SITE_ROOT.DS.'..'.DS.'uploads/student';
  ...
  public function upload($file)
  {
    ...
    elseif(!$this->file_ext($file['name'])):
      $this->errors[] = 'Only image files are allowed (e.g., JPG, PNG, GIF).';
      return false;
    else:
 ...
 public function process(){
    ...
    elseif(empty($this->fileName) || empty($this->fileTempPath)):
      ...
      return false;
    elseif(!is_writable($this->studentPath)):
      $this->errors[] = $this->studentPath." Must be writable!!!.";
      return false;
    elseif(file_exists($this->studentPath."/".$this->fileName)):
 ...
 /*--------------------------------------------------------------*/
  public function process_media(){
    ...
    if(!is_writable($this->studentPath)){
        $this->errors[] = $this->studentPath." Must be writable!!!.";
        return false;
      }

    if(file_exists($this->studentPath."/".$this->fileName)){
      $this->errors[] = "The file {$this->fileName} already exists.";
      return false;
    }

    if(move_uploaded_file($this->fileTempPath,$this->studentPath.'/'.$this->fileName))
    {
...
public function process_student($id) {
    if (!empty($this->errors)) {
        return false;
    }
    if (empty($this->fileName) || empty($this->fileTempPath)) {
    ...
    if (!$id) {
        $this->errors[] = "Missing student id.";
        return false;
    }
    ...
    if ($this->student_image_destroy($id)) {
        if (move_uploaded_file($this->fileTempPath, $this->studentPath . '/' . $this->fileName)) {
            if ($this->update_studentImg($id)) {
                unset($this->fileTempPath);
                return true;
            }
        } else {
...
/*--------------------------------------------------------------*/
/* Function for Update student image
/*--------------------------------------------------------------*/
private function update_studentImg($id) {
    global $db;
    $sql = "UPDATE student SET";
    $sql .= " student_image='{$db->escape($this->fileName)}'";
    $sql .= " WHERE id='{$db->escape($id)}'";
    $result = $db->query($sql);
    return ($result && $db->affected_rows() === 1 ? true : false);
}
...
/* Function for Delete old image
/*--------------------------------------------------------------*/
public function student_image_destroy($id) {
    $image = find_by_id('student', $id);
    if ($image['student_image'] === 'defualt.png') {
        return true;
    } else {
        unlink($this->studentPath . '/' . $image['student_image']);
        return true;
    }
}
...
/*--------------------------------------------------------------*/
   public function media_destroy($id,$file_name){
     ...
     if(delete_by_id('media',$id)){
         unlink($this->studentPath.'/'.$this->fileName);
         return true;
     } else {
